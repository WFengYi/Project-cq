...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Orders> implements OrderService {
    ...
    public OrderSubmitVO submitOrder(OrdersSubmitDTO ordersSubmitDTO) {
        ...
        for (ShoppingCart cart : shoppingCartList) {//得到购物车列表
            OrderDetail orderDetail = new OrderDetail();
            BeanUtil.copyProperties(cart, orderDetail);
            orderDetail.setOrderId(orders.getId());//设置明细表关联的订单id
            orderDetailList.add(orderDetail);//添加数据到集合里
        }
        orderDetailMapper.insertBatch(orderDetailList);
        //4.删除购物车
        QueryWrapper wrapper = new QueryWrapper();
        wrapper.eq("user_id", userId);
        shoppingCartMapper.delete(wrapper);//删除当前用户的购物车
        //封装vo返回结果
...
//        );
    ...
    public void paySuccess(String outTradeNo) {
        ...
        Orders orders = Orders.builder()
                ...
                .address(addressBook.getDetail() + " " + addressBook.getProvinceName() + " " + addressBook.getCityName() + " " + addressBook.getDistrictName())
                .tablewareNumber(ordersDB.getTablewareNumber())
                .tablewareStatus(ordersDB.getTablewareStatus())
                .orderTime(ordersDB.getOrderTime())
                .number(ordersDB.getNumber())
                .estimatedDeliveryTime(ordersDB.getEstimatedDeliveryTime())
